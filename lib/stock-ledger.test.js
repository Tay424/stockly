...
test("startOfLocalWeek returns Monday 00:00 for the containing week", () => {
  // Wednesday 2026-08-05 → Monday 2026-08-03
  const wed = new Date(2026, 7, 5, 15, 30, 0);
  ...
  // Already Monday stays that Monday
  const mon = new Date(2026, 7, 3, 8, 0, 0);
  assert.equal(startOfLocalWeek(mon).getTime(), startOfLocalDay(mon).getTime());

  // Sunday maps to previous Monday
  const sun = new Date(2026, 7, 9, 12, 0, 0);
  assert.equal(startOfLocalWeek(sun).getDate(), 3);
});
