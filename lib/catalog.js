...
import { nhavaQuantityOnSale, repriceSaleNhava } from "./nhava-reprice.js";
...
export async function listCategories() {
  ...
  return rows.map(({ _id, ...rest }) => ({
    ...
    retailPackQty: rest.retailPackQty ?? 0,
    retailPackPriceCents: rest.retailPackPriceCents ?? 0,
  }));
}
...
export async function createCategory({
  ...
  await categories().insertOne({
    ...
    retailPackQty: Number(retailPackQty) || 0,
    retailPackPriceCents: Number(retailPackPriceCents) || 0,
    createdAt: now,
    updatedAt: now,
  });
}
...
const serializeSale = ({ _id, productId, soldBy, distributorId, lines, packs, retailPacks, retailLines, ...rest }) => ({
  id: String(_id),
  ...
  packs: Array.isArray(packs)
    ...
    : undefined,
  retailPacks: Array.isArray(retailPacks)
  ...
  backfilledAt: rest.backfilledAt?.toISOString?.() ?? rest.backfilledAt ?? null,
...
export async function listSellableProducts() {
  ...
  return rows.map(({ _id, categoryId, category, ...rest }) => ({
    ...
    retailPackQty: category?.retailPackQty ?? 0,
    retailPackPriceCents: category?.retailPackPriceCents ?? 0,
    ...rest,
    discountStartsAt: rest.discountStartsAt?.toISOString() ?? null,
    discountEndsAt: rest.discountEndsAt?.toISOString() ?? null,
  }));
 ...
 * `soldAt`: optional Date — historical sale time (admin backfill); defaults to now
 */
export async function recordSaleReceipt({ cartLines, saleMeta, client = null, soldAt = null }) {
  const ids = [
    ...new Set(
      (cartLines ?? [])
  ...
  const categoriesById = new Map(
    categoryRows.map((row) => [
      String(row._id),
      {
        ...
        retailPackQty: row.retailPackQty ?? 0,
        retailPackPriceCents: row.retailPackPriceCents ?? 0,
      },
    ]),
  );
  ...
  const at =
    soldAt instanceof Date && !Number.isNaN(soldAt.getTime()) ? soldAt : wallNow;
  const isBackfill = soldAt instanceof Date && !Number.isNaN(soldAt.getTime());
  const decremented = [];
  ...
  for (const line of priced.stockLines) {
    ...
    const updated = await products().findOneAndUpdate(
      ...
      { $inc: { stock: -line.quantity }, $set: { updatedAt: wallNow } },
      { returnDocument: "after" },
    );
    if (!updated) {
      ...
      for (const prior of decremented) {
        await products().updateOne(
          { _id: prior._id },
          { $inc: { stock: prior.quantity }, $set: { updatedAt: wallNow } },
        );
      }
      return {
        ok: false,
  ...
  const retailPacksForDb = (priced.retailPacks ?? []).map((pack) => ({
    ...pack,
    categoryId: toObjectId(pack.categoryId),
    contributions: pack.contributions.map((c) => ({
      ...c,
  ...
  const { insertedId } = await sales().insertOne({
    ...
    createdAt: at,
    ...(isBackfill
      ? { source: "backfill", backfilledAt: wallNow }
      : {}),
  });
...
function withObjectIdsForSalePricing(sale, nhavaProductId, nhavaCategoryId) {
  const toId = (value) => {
    if (value == null || value === "") return value;
    if (value instanceof ObjectId) return value;
    return toObjectId(String(value)) ?? value;
  };
  ...
  return {
    ...
    // Keep top-level productId pointing at first line when present.
...
/**
 * Ensure Nhava Salt has its own category (retail 1@$30, wholesale 5@$60),
 * move the product there, and reprice every historical sale that includes it.
 * Stock movements are left unchanged — only money/category attribution updates.
 */
export async function repriceNhavaSaltHistoricalSales({ force = false } = {}) {
  const now = new Date();
  ...
  let category = await categories().findOne({ name: NHAVA_NAME });
  if (!category) {
    ...
    category = await categories().findOne({ _id: insertedId });
  } else {
  ...
  const product = await products().findOne({ name: NHAVA_NAME });
  if (!product) {
    return { ok: false, reason: "Nhava Salt product was not found." };
  }
  ...
  // Drop Nhava from Aura Salts description if still listed there.
  const aura = await categories().findOne({ name: /aura salts?/i });
  ...
  for (const row of candidates) {
    ...
    const forDb = withObjectIdsForSalePricing(result.sale, nhavaProductId, String(category._id));
