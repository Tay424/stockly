...
import { endOfLocalDay, startOfLocalDay, startOfLocalWeek } from "./stock-ledger.js";
...
export async function sellerStats(userId, now = new Date()) {
  ...
  const [today, week, allTime, spentRow] = await Promise.all([
    sumSales({ soldBy: userId, createdAt: { $gte: startOfDay } }),
    sumSales({ soldBy: userId, createdAt: { $gte: weekStart } }),
