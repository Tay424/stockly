...
export function findDiscrepancies(sales, movements, window = {}) {
  ...
  for (const sale of sales) {
    ...
    const row = touch(sale.productId, sale.productName);
    if (createdInWindow && status !== SALE_STATUS.voided) {
      row.soldUnits += sale.quantity ?? 0;
    }
    // Voided sales still "sold" for the day they were created — until voided.
    // For product rollup on the void day, count voided units separately.
    if (createdInWindow && status === SALE_STATUS.voided) {
      // Sale + void same day: net should be zero; count both sides.
      row.soldUnits += sale.quantity ?? 0;
      row.voidedUnits += sale.quantity ?? 0;
