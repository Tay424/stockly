...
/**
 * Parse receive timestamps from Date, datetime-local (`YYYY-MM-DDTHH:mm`),
 * or legacy date-only (`YYYY-MM-DD`). Returns null when invalid.
 */
export function parseReceivedAt(value, fallback = new Date()) {
  if (value == null || value === "") {
...
export async function receiveStock({
  ...
  const when = parseReceivedAt(receivedAt);
  if (!when) {
    return { ok: false, reason: "Receive date and time must be valid." };
  }
...
export async function listStockReceives({
  monthKey = null,
  createdBy = null,
  limit = 100,
} = {}) {
